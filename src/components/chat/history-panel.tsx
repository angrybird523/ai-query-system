/**
 * 文件名: history-panel.tsx
 * 功能描述: 历史对话侧边栏面板，展示近30天对话记录列表，支持新建对话、切换查看、删除确认。
 * 主要导出: HistoryPanel
 */
  ...
  return (
    <div className="w-72 bg-[#F8FAFC] border-r border-[#E2E8F0] flex flex-col shrink-0 relative">
      ...
      <div className="h-16 flex items-center px-4 border-b border-[#E2E8F0] shrink-0">
        <button onClick={onClose} className="p-1.5 rounded-md hover:bg-[#E2E8F0] transition-colors mr-2">
        ...
        <div className="space-y-0.5">
          {conversations.map((conv) => (
            <div key={conv.id} className="group relative">
              ...
              {/* 悬停显示的删除按钮 */}
              <button onClick={(e) => handleDeleteClick(e, conv.id)}
      ...
      {deleteConfirmId && (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-5 shadow-lg max-w-[280px] w-full mx-4">
            ...
            <p className="text-sm text-[#64748B] mb-4">确定删除该对话记录吗？此操作不可撤销。</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteConfirmId(null)}
                className="px-3 py-1.5 text-sm text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0] rounded-lg transition-colors">取消</button>
              <button onClick={handleConfirmDelete}
                className="px-3 py-1.5 text-sm text-white bg-[#EF4444] hover:bg-[#DC2626] rounded-lg transition-colors">删除</button>
            </div>
          </div>
        </div>
      )}
